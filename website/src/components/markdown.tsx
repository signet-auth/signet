...
function TocLink({
    ...
    return (
        ...
        >
            ...
            <span style={{ overflowWrap: 'anywhere', fontFamily: 'var(--font-primary)', flex: 1 }}>
                {item.label}
            </span>
            {chevron && (
                <span
...
export function TableOfContents({ items, logo: _logo }: { items: FlatTocItem[]; logo?: string }) {
    ...
    const [expanded, setExpanded] = useState<Set<string>>(() => {
        const first = items.find((i) => {
            return expandableHrefs.has(i.href);
        });
        ...
        >
            ...
            >
                {visibleItems.map((item, i) => {
                    ...
                    const highlightedHref = isSearchActive
                        ? searchState.focusableHrefs?.[highlightedIndex]
                        : undefined;
                    ...
                    return (
                        <div
                            key={item.href}
                            style={
                                item.visualLevel === 0 && i > 0 ? { marginTop: '6px' } : undefined
                            }
                        >
                            <TocLink
                                item={item}
