...
export function generateConfigYaml(options: InitOptions): string {
  ...
  const modeLine = options.mode === 'browserless'
    ? 'mode: browserless                          # No browser on this machine — use sig sync pull, --cookie, or --token'
    : 'mode: browser                              # Use "browserless" on remote/headless machines without a browser';
