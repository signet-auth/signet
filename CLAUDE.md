...
8. **Credential types**: Discriminated union on `type` field: `'cookie' | 'bearer' | 'api-key' | 'basic'`. Cookie and bearer credentials may include `xHeaders?: Record<string, string>` for captured extra headers and `localStorage?: Record<string, string>` for extracted browser localStorage values.
